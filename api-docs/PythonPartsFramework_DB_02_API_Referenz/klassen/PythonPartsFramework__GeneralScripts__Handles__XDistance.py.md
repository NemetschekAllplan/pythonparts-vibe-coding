---
title: "XDistance"
source: "PythonPartsFramework\GeneralScripts\Handles\XDistance.py"
type: "class"
category: "02_API_Referenz"
tags:
  - handle
  - script
related:
  -
last_updated: "2026-02-20"
---


# XDistance

> **Pfad:** `PythonPartsFramework\GeneralScripts\Handles\XDistance.py`  
> **Typ:** Klasse/Modul  
> **Tags:** `handle`, `script`

## Übersicht

implementation of the x distance modification

## Abhängigkeiten

- `BaseHandlePropUpdate`
- `math`

## Klassen

### `XDistance`

modify the x distance
    

#### Methoden

| Methode | Parameter | Rückgabe | Beschreibung |
|---|---|---|---|
| `__call__` | `self` | `None` | execute the value update          |

## Funktionen

Keine Funktionen vorhanden.

## Quellcode

<details>
<summary>Vollständiger Code anzeigen</summary>

```python
""" implementation of the x distance modification
"""

import math

from .BaseHandlePropUpdate import BaseHandlePropUpdate

class XDistance(BaseHandlePropUpdate):
    """ modify the x distance
    """

    def __call__(self):
        """ execute the value update
        """

        self.update_property_value(math.fabs(self._dist_pnt.X) if self._is_abs_value else self._dist_pnt.X)

```

</details>