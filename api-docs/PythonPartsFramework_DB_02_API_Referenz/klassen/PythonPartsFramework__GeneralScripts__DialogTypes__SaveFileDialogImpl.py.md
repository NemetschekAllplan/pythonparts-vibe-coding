---
title: "SaveFileDialogImpl"
source: "PythonPartsFramework\GeneralScripts\DialogTypes\SaveFileDialogImpl.py"
type: "class"
...
> **Pfad:** `PythonPartsFramework\GeneralScripts\DialogTypes\SaveFileDialogImpl.py`  
> **Typ:** Klasse/Modul  
> **Tags:** `dialog`, `script`

## Übersicht

implementation of the save file dialog
...
class SaveFileDialogImpl(FileDialog):
    """ implementation of the save file dialog
    """
    ...
    def show(build_ele      : BuildingElement,
        ...
        if (index := SaveFileDialogImpl.get_index(build_ele, value_ctrl_prop, name)) is None:
            prop.value = AllplanUtil.FileDialog.AskSaveFile(prop.value, value_ctrl_prop.text,
                                                            value_ctrl_prop.value_list, value_ctrl_prop.value_list.split("|")[0],
                                                            SaveFileDialogImpl.get_default_directories(value_ctrl_prop))
        else:
            prop.value[index] = \
                AllplanUtil.FileDialog.AskSaveFile(prop.value[index], value_ctrl_prop.text,
                                                   value_ctrl_prop.value_list, value_ctrl_prop.value_list.split("|")[0],
                                                   SaveFileDialogImpl.get_default_directories(value_ctrl_prop))
